...
module Isuconp
  class App < Sinatra::Base
    ...
    get '/@:account_name' do
      ...
      posts = make_posts(results)
      comments_by_user = db.prepare('SELECT * FROM `comments` WHERE `user_id` = ?').execute(
        user[:id]
      )
      comment_count = comments_by_user.count
      ...
      if post_count > 0
        ...
        comments_by_post = db.prepare("SELECT * FROM `comments` WHERE `post_id` IN (#{placeholder})").execute(
          *post_ids
        )
        commented_count = comments_by_post.count
      end
